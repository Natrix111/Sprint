const labelFileSelector = document.getElementById('label-file');
...
function reset(event) {
    ...
    formApplication.innerHTML = '';
    clearForm.style.display = 'none';
    fileSelector.style.display = 'block';
    fileSelector.value = '';
    labelFileSelector.innerText = 'Загрузите JSON:';
}
...
function parseObjToHtml(obj, filename) {
    ...
    formApplication.append(title);
    formApplication.append(form);
...
function createFormTitle(string) {
...
function createInputElement(input, reserveId = 0) {
    const regex = /[^\w]|[0-9]/g;
    const isLabelExist = input.hasOwnProperty('label');
    let id = isLabelExist && !!input.label.replace(regex, '') ? input.label : reserveId;

    let {type, mask, technologies, filetype, checked, colors,} = input.input;
        ...
        case 'technology':
            ...
            multipleCheckboxElements = arrayForCheckbox.map((value) => {
                ...
                return createHtmlWrapper(
                    {label: labelMultipleElement, input: inputMultipleElement}, 'checkbox',
                );
            });
